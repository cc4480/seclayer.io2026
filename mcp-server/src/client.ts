...
export async function scan(
  ...
  if (response.status === 401) {
    return {
      ok: false,
      kind: "unauthorized",
      message: `Authentication or credit failure: the Seclayer API key is invalid/inactive, or the account is out of credits (the backend does not distinguish these two cases). Details: ${detail}. Verify the key and check your credit balance at https://seclayer.app.`,
    };
  }
...
export async function startAutofixSession(
  ...
  if (response.status === 401) {
    return { ok: false, kind: "unauthorized", message: `Authentication or credit failure starting autofix: ${detail}. Verify the key and check your credit balance at https://seclayer.app.` };
  }
  if (response.status === 429) {
    const retryAfterHeader = response.headers.get("retry-after");
    const retryAfterSeconds = retryAfterHeader ? Number(retryAfterHeader) : undefined;
