...
> **LIVE END-TO-END SCAN — dashboard path + BOLA `[VERIFIED ✓ — 2026-07-12]`:** The `/api/scans` path was verified separately with two owned identities. `POST /api/scans` with `activeProbes: true` and `bolaIdentities` for the vulnerable orders API (`tok-alice`/`1001` + `tok-bob`/`1002`) → async worker (`queued → complete`), score **10/100 (critical), 11 findings, 6 PROVEN receipts** — the five RED_TEAM ones above **plus a PROVEN API_SEC `differential` BOLA**. The BOLA receipt is a genuine two-identity proof: **attack** — identity A (`Bearer tok-alice`) requested `GET /api/orders/1002` and received B's data (`bob@vulnshop.test`, quoted verbatim); **baseline** — A reading its own order captured; **control** — an unauthenticated request returned `HTTP/1.1 401 Unauthorized`, proving the endpoint is auth-gated (a real cross-tenant break, not a public resource).

> **LIVE END-TO-END SCAN — GraphQL introspection `[VERIFIED ✓ — 2026-07-12]`:** The last remaining active module, verified live after adding a faithful introspection endpoint to the test target (`test-targets/vulnerable-app.mjs` now answers `POST /graphql` with a real `data.__schema.types` result). A dashboard scan produced a **PROVEN** `API_SEC` finding *GraphQL Schema Introspection Exposed*, method `introspection`, **severity `high` (correctly not critical — disclosure, not exploitation)**, quoting `"__schema"` verbatim and dumping the 6-type schema (`Query, Mutation, User, Order, Payment, AdminSecret`) with a replayable `curl -X POST .../graphql`. **With this, all eight active modules are confirmed firing against a real running target through the live HTTP API** — SQLi, XSS, cmd-injection, reflected + blind SSRF, BOLA, GraphQL introspection, and (per the test suite) the discovered-parameter fuzzer.
