...
1. "aiSummary": A direct, plain-English, professional executive summary paragraph (3-5 sentences) summarizing overall posture, potential risks, and urgency level. Speak with the authority of an active principal cybersecurity assessor. Do NOT use fake placeholders like "example.com", you MUST explicitly mention the target url "${url}" in your summary. Do NOT use markdown links.
   CRITICAL: do NOT state a numeric posture score or a letter grade anywhere in your prose — no "85/100", no "scores 85", no "grade B". The interface computes and displays the score itself, and recalculates it whenever the reader suppresses a finding as a false positive, so any number you write here would contradict the score shown on screen. Describe severity in words ("critical", "weak", "solid") instead.
2. "executiveBreakdown": A detailed, multi-part breakdown for a developer/engineering-lead audience — this is the DEEP report, not a repeat of "aiSummary". Object with exactly these fields:
   - "overview": 2-4 sentences of context on what was tested and the overall posture story (not identical wording to "aiSummary"). The same rule applies here: never quote a numeric score or letter grade.
...
3. "findings": An array corresponding to the detected issues, rewritten with clearer titles/descriptions of how an attacker would exploit the issue. Each item MUST have exactly these fields:
   ...
   - "agentPrompt": a ready-to-paste MASTER PROMPT a developer hands straight to their own AI coding agent — it must work identically with any AI coding agent on any platform, so use only plain, provider-neutral markdown (no agent-specific syntax, slash commands, or XML tags). Write it as a self-contained engineering instruction and do NOT mention Seclayer, "the scan", or this report inside the prompt text. Follow this exact section structure, in order:
       ...
       "## Recommended fix" — the concrete secure change, tailored to the detected stack (${techContext}) where that narrows down where to look; name the exact vulnerable pattern to search for and its secure replacement.
       "## Work it in this order" — a numbered list: 1) Locate the responsible route/handler/config (note the scan is black-box and never saw the source, so search by behavior); 2) Confirm it's real locally and stop + report a likely false positive if it isn't; 3) Fix the root cause with the project's existing conventions, fixing every instance if the pattern repeats; 4) Add a regression test that fails before and passes after; 5) Verify by running the full test suite and re-exercising the request.
       "## Guardrails" — do not weaken other security controls to pass, keep the diff minimal, never hardcode secrets, pin patched dependency versions.
       "## Done when" — a short checklist ("- [ ] ...") covering root-cause fixed, regression test passing, suite green, and a summary of what changed.
