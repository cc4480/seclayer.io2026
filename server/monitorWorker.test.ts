...
async function backdateToDue(targetId: string, iso = new Date(Date.now() - 1000).toISOString()) {
  (await db.markMonitoredScanned(targetId, iso, iso));
}

test('a due target with credits and a safe URL launches a scan and deducts one credit', async () => {
  const u = (await db.getOrCreateUser(`mw-success-${Date.now()}@test.io`));
  ...
  assert.equal((await db.getUser(u.id))!.credits, creditsBefore - 1);
...
test('a target not yet due is left completely untouched', async () => {
  const u = (await db.getOrCreateUser(`mw-not-due-${Date.now()}@test.io`));
  const target = (await db.addMonitoredTarget(u.id, SAFE_TARGET, 7)); // freshly added -> scheduled in the future
  const creditsBefore = (await db.getUser(u.id))!.credits;
  ...
  assert.equal((await db.listScans(u.id)).length, 0);
  const unchanged = (await db.listMonitoredTargets(u.id)).find((t) => t.id === target.id)!;
  assert.ok(!unchanged.lastScannedAt);
});

test('a due target with no credits is skipped, retried next tick, and records why', async () => {
  const u = (await db.getOrCreateUser(`mw-no-credits-${Date.now()}@test.io`));
...
test('a due target with an unsafe URL is deferred to its next cadence and records why, without spending a credit', async () => {
  const u = (await db.getOrCreateUser(`mw-unsafe-${Date.now()}@test.io`));
  const target = (await db.addMonitoredTarget(u.id, UNSAFE_TARGET, 7));
  backdateToDue(target.id);
  const creditsBefore = (await db.getUser(u.id))!.credits;
  ...
  assert.equal(launched, false);
  assert.equal((await db.getUser(u.id))!.credits, creditsBefore, 'an unsafe target must never spend a credit');
  assert.equal((await db.listScans(u.id)).length, 0);
  const updated = (await db.listMonitoredTargets(u.id)).find((t) => t.id === target.id)!;
...
test('a successful launch clears a lingering lastError from a previous failed tick', async () => {
  const u = (await db.getOrCreateUser(`mw-recover-${Date.now()}@test.io`));
  const target = (await db.addMonitoredTarget(u.id, SAFE_TARGET, 7));
  backdateToDue(target.id);
  (await db.markMonitoredError(target.id, 'Skipped: insufficient credits.'));
  assert.ok((await db.listMonitoredTargets(u.id)).find((t) => t.id === target.id)!.lastError);
...
test('a paused target is never scanned even when its next run is due', async () => {
  const u = (await db.getOrCreateUser(`mw-paused-${Date.now()}@test.io`));
  const target = (await db.addMonitoredTarget(u.id, SAFE_TARGET, 7));
  backdateToDue(target.id); // would be due
  (await db.setMonitoredPaused(u.id, target.id, true));
  const creditsBefore = (await db.getUser(u.id))!.credits;
  ...
  assert.equal((await db.getUser(u.id))!.credits, creditsBefore, 'no credit spent on a paused target');
  assert.equal((await db.listScans(u.id)).length, 0);
});

test('overlapping ticks do not double-process the same due target', async () => {
  const u = (await db.getOrCreateUser(`mw-overlap-${Date.now()}@test.io`));
  ...
  const target = (await db.addMonitoredTarget(u.id, 'https://example.com', 7));
  backdateToDue(target.id);
